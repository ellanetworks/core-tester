...
static __always_inline int build_udp_header(void *data, void *data_end)
{
    ...
    struct
    {
        ...
        .src = ((struct iphdr *)(data + sizeof(struct ethhdr)))->saddr,
        .dst = ((struct iphdr *)(data + sizeof(struct ethhdr)))->daddr,
        .zero = 0,
        .proto = IPPROTO_UDP,
        .len = udph->len,
    };

    __u64 csum = bpf_csum_diff(
        0, 0,
        (__be32 *)udph, udp_len,
        0);
    csum = bpf_csum_diff(
...
static __always_inline int build_gtp_header(void *data, void *data_end)
{
    ...
    gtp[1] = 0xFF; // T-PDU
    *(__u16 *)(gtp + 2) =
        bpf_htons((__u16)((__u8 *)data_end - (gtp + GTP_HDR_LEN)));
